...
// import { IconChevronsLeft, IconMenu2, IconX } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <Layout fixed className={navOpened ? 'h-svh' : ''}>
        ...
        <Layout.Header sticky className='z-50 flex justify-between px-4 py-3 shadow-sm md:px-4'>
          <div className={`flex items-center ${!isCollapsed ? 'gap-2' : ''}`}>
            ...
            <div className={`flex flex-col justify-end truncate ${isCollapsed ? 'invisible w-0' : 'visible w-auto'}`}>
              <span className='text-2xl font-extrabold'>TECHEDU</span>
              <span className='text-xs font-medium'>System Management</span>
            </div>
          </div>
        ...
        >
          <ArrowLeft className={`h-5 w-5 ${isCollapsed ? 'rotate-180' : ''}`} />
        </Button>
      </Layout>
    </aside>
  )
