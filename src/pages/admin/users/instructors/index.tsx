...
const mockData: ITypeInstructorTable[] = [
  {
    _id: '1',
  ...
  {
    _id: '5',
    instructorAvatar: 'https://randomuser.me/api/portraits/women/50.jpg',
    courses: 50,
...
const columns = [
  ...
  {
    id: 'totalStudents',
    header: () => <p className=''>Total Students</p>,
    cell: ({ row }: { row: any }) => {
      return <p className='font-medium '>{row.original.totalStudents}</p>
    },
    enableSorting: false,
    enableHiding: false
  },
  {
    id: 'totalRevenue',
    header: () => <p className=''>Total Revenue</p>,
    cell: ({ row }: { row: any }) => {
      return <p className='font-medium '>${row.original.totalRevenue}</p>
    },
    enableSorting: false,
    enableHiding: false
  },
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end' className='rounded-xl min-w-[160px] py-2'>
              <DropdownMenuItem className='flex items-center w-full p-3 mb-2 text-sm rounded-lg cursor-pointer hover:bg-neutral-silver focus:outline-none'>
                Edit instructor's detail
              </DropdownMenuItem>
              <DropdownMenuItem className='flex items-center w-full p-3 mb-2 text-sm rounded-lg cursor-pointer hover:bg-neutral-silver focus:outline-none'>
                View instructor's detail
              </DropdownMenuItem>
              <DropdownMenuItem className='flex items-center w-full p-3 mb-2 text-sm rounded-lg cursor-pointer hover:bg-neutral-silver focus:outline-none'>
                Ban Account
              </DropdownMenuItem>
              <DropdownMenuItem className='flex items-center w-full p-3 mb-2 text-sm rounded-lg cursor-pointer hover:bg-neutral-silver focus:outline-none'>
                View instructor's detail revenue
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
...
export default function Instructors() {
  const instructorTable = useReactTable({
    data: mockData,
    columns,
    ...
    <Layout>
      ...
      <Layout.Body>
        ...
        <div className='grid gap-4 mt-3 lg:grid-cols-3'>
          ...
          <Card>
            <CardHeader className='flex flex-row items-center justify-between pb-2 space-y-0'>
              <CardTitle className='text-sm font-medium'>Activated Instructors</CardTitle>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                viewBox='0 0 24 24'
                fill='none'
        ...
        <div className='mt-5 w-full overflow-auto h-[500px] rounded-lg no-scrollbar'>
          <Table className='w-full h-full overflow-auto'>
            <TableHeader className='sticky top-0 z-20 bg-white border-b sm:py-4 tb:-top-3'>
              {instructorTable.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody className='h-[50px] overflow-y-auto'>
              {instructorTable?.getRowModel()?.rows?.length ? (
                instructorTable?.getRowModel()?.rows?.map((row) => (
                  <TableRow
                    key={row.id}
